...
{
	Botan::secure_vector<std::uint8_t> data_vector{data.begin(), data.end()};
...
inline void synchronize(user& lhs, user& rhs)
{
	if (lhs.get_blockchain().size() < rhs.get_blockchain().size())
		lhs.get_blockchain() = rhs.get_blockchain();
	else
		rhs.get_blockchain() = lhs.get_blockchain();
}
