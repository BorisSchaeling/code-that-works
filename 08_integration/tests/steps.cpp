...
GSTEPS("Integration*")
{
	...
	std::vector<char> client_stdout(64);
	...
	Given("a client with the blockchain", "data") = [&](const testing::Table& table)
	{
		std::string blockchain;
		for (const auto& row : table)
			blockchain += row.at("data") + ',';
		boost::algorithm::trim_right_if(blockchain, boost::algorithm::is_any_of(","));
		client = boost::process::child{ "../src/exe --client " + blockchain, boost::process::std_out > boost::asio::buffer(client_stdout), io_context };
	};
	...
	Then("the server's blockchain contains", "data") = [&](const testing::Table& table)
	{
		std::string blockchain;
		for (const auto& row : table)
			blockchain += row.at("data") + ',';
	...
	Then("the client's blockchain contains", "data") = [&](const testing::Table& table)
	{
		std::string blockchain;
		for (const auto& row : table)
			blockchain += row.at("data") + ',';
		blockchain.back() = '\n';
		EXPECT_EQ(blockchain, std::string_view(client_stdout.data(), blockchain.size()));
	};
}
